...
class ConstructorPage(BasePage):
    ...
    def click_close_modal(self):
        ...
        self.wait_for_invisibility(ConstructorPageLocators.MODAL_DETAILS)
    ...
    def is_modal_details_invisible(self):
        try:
            self.wait_for_invisibility(ConstructorPageLocators.MODAL_DETAILS, time=3)
            return True
        except Exception:
            return False
    ...
    def drag_and_drop_ingredient_to_basket(self):
        ...
        while time.time() < deadline:
            if self.get_ingredient_counter_value() > 0:
                return
            time.sleep(0.3)
        raise AssertionError("Счётчик ингредиента не обновился после drag-and-drop")
    ...
    def _js_drag_and_drop(self, source, target):
        ...
        """
        self.execute_script(js_drag, source, target)
    ...
    def is_order_submitted_modal_visible(self):
        self.wait_for_visibility(ConstructorPageLocators.ORDER_SUBMITTED_MODAL, time=15)
        return True
